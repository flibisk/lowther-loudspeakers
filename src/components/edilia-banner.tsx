import Image from "next/image";
import Link from "next/link";
import { Button } from "@/components/ui/button";

export function EdiliaBanner() {
  return (
    <section className="relative w-full h-[600px] overflow-hidden">
      {/* Background Image */}
      <div className="absolute inset-0">
        <Image
          src="/images/speakers/edilia/hero/Edilia-mid-banner1.webp"
          alt="Lowther Edilia speaker in elegant listening room setting"
          fill
          className="object-cover"
          priority
        />
        
        {/* Gradient overlay from left to right for text readability */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/40 to-transparent"></div>
      </div>

      {/* Content */}
      <div className="relative z-10 h-full flex items-center">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 w-full">
          <div className="max-w-lg">
            {/* Top separator */}
            <div className="flex items-center mb-4">
              <div className="w-8 h-px bg-white mr-3"></div>
              <span className="text-sm tracking-wider uppercase text-white/80">
                BESPOKE SPEAKERS
              </span>
            </div>

            {/* Main title */}
            <h2 className="font-display text-4xl lg:text-5xl leading-tight mb-6" style={{ color: '#c59862' }}>
              Lowther Edilia
            </h2>

            {/* Description */}
            <p className="text-lg leading-relaxed mb-8 text-white">
              A compact dual-driver masterpiece, the Edilia delivers Lowther's 
              legendary clarity with room-tunable bass. Bringing full-range 
              realism to smaller spaces without compromise.
            </p>

            {/* Learn More button */}
            <Link href="/speakers/edilia">
              <Button 
                variant="white" 
                size="lowther"
                className="bg-white text-black hover:bg-[#c59862] hover:text-white transition-all duration-300"
              >
                LEARN MORE
              </Button>
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
}
