...
/**
 * Test API route for abandoned cart Email 1
 * 
 * Usage:
 * POST /api/test-abandoned-cart-email
 * Body: { "email": "your-email@example.com" }
 * 
 * Or GET /api/test-abandoned-cart-email?email=your-email@example.com
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate email HTML
    const html = buildQueueEmail(cartUrl, testCartItems);
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: testEmail,
      replyTo: fromEmail,
      subject: 'Test: Your place in our build queue is reserved',
      html,
    });
    ...
    if (!testEmail || !testEmail.includes('@')) {
      return NextResponse.json(
        { 
          ...
          usage: 'GET /api/test-abandoned-cart-email?email=your-email@example.com'
        },
        { status: 400 }
      );
    }
